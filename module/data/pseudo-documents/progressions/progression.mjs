...
export default class Progression extends PseudoDocument {
  ...
  static get metadata() {
    return {
      ...
      defaultImage: "systems/artichron/assets/icons/advancement.svg", // TODO
      embedded: {},
      sheetClass: null,
    };
  }
  ...
  get items() {
    return this.document.items.filter(item => {
      const { progressionId } = item.getFlag("artichron", "advancement") ?? {};
  ...
  async #confirmDeletion() {
    return artichron.applications.api.Dialog.confirm({ content: "Really undo progression?" });
  }
